...
export const MovieDetail = () => {
  //get parameters for movie details to get the movie id.
  const params = useParams();

  //upate the movie object
  ...
  useEffect(() => {
    async function movie() {
      const resp = await fetch(
        `https://api.themoviedb.org/3/movie/${params.id}?api_key=b52a9b5a70358ea8ae6ea60ba12f2bde`
      );
          ...
          {movie.genres ? (
            <p className="flex flex-wrap gap-2">
              {movie.genres.map((genre) => (
                <span
                  className="mr-2 border border-gray-200 rounded p-2 dark:border-gray-600"
                  key={genre.id}
                >
                  {genre.name}
                </span>
